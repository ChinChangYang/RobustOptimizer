function [xmin, fmin, out] = umoeas_c(fitfun, lb, ub, maxfunevals, options)
% UMOEAS_C United Multi-Operator Evolutionary Algorithms (variant C)
% UMOEAS_C(fitfun, lb, ub, maxfunevals) minimize the function fitfun in
% box constraints [lb, ub] with the maximal function evaluations
% maxfunevals.
% UMOEAS_C(..., options) minimize the function by solver options.
if nargin <= 4
	options = [];
end

defaultOptions.NP1 = 300;
defaultOptions.NP2 = 150;
defaultOptions.CS = 50;
defaultOptions.MixStageFactor = 0.1;
...
CS = options.CS;
MixStageFactor = options.MixStageFactor;
...
out = initoutput(RecordPoint, D, NP1, maxfunevals, 'xstd1', 'xstd2');
...
% Initialize variables
MixStage = MixStageFactor * maxfunevals;
bestSolver = 1;
success = zeros(CS, 2);
if fx1(1) < fx2(1)
	fmin = fx1(1);
...
while true
	...
	elseif period == CS + CS + 1 && counteval <= MixStage
