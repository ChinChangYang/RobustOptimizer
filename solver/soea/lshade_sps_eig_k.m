function [xmin, fmin, out] = lshade_sps_eig_k(fitfun, lb, ub, maxfunevals, options)
% LSHADE_SPS_EIG_K L-SHADE algorithm with SPS+EIG framework (variant K)
% LSHADE_SPS_EIG_K(fitfun, lb, ub, maxfunevals) minimize the function fitfun in
% box constraints [lb, ub] with the maximal function evaluations
% maxfunevals.
% LSHADE_SPS_EIG_K(..., options) minimize the function by solver options.
if nargin <= 4
	options = [];
end

defaultOptions.NP = 550;
...
defaultOptions.initial.strategy = [];
...
if ~isempty(options.initial)
	...
	strategy = options.initial.strategy;
else
	X		= [];
	fx		= [];
	A		= [];
...
if isequal(strategy, 'separable')
	CRmax = CRmin;
elseif isequal(strategy, 'rotated')
	CRmin = CRmax;
end
...
while true
	...
	% Check strategy
	if (mean(MER) <= 0 || mean(MER) >= 1.5) && ...
			isequal(strategy, 'initial') && ...
			counteval <= 0.2 * maxfunevals
		
		% Begin: Reinitialize all variables
		for i = 1 : NP
			X(:, i) = lb + (ub - lb) .* rand(D, 1);
		end
		...
		iSP = 1;
		countstagnation = 0;
		% End: Reinitialize all variables
