function f=benchmark_func(x, varargin)
global initial_flag
persistent fhd f_bias func_num x_dim
...
if isempty(func_num)
	...
	x_dim = numel(x);
	initial_flag = 0;
elseif func_num ~= varargin{1} || x_dim ~= numel(x)
	func_num = varargin{1};
	x_dim = numel(x);
	initial_flag = 0;
end
...
o2=repmat(o,ps,1);
x=(abs(x-o2)<0.5).*x+(abs(x-o2)>=0.5).*(round(x.*2)./2);
